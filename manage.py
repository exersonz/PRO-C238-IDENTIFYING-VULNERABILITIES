# importing necessary modules from Flask, datetime, csv, os, and the application's models
...
# creating a FlaskGroup instance to add custom CLI commands
cli = FlaskGroup(create_app=create_app)

# sample user data to be seeded into the Users table
user_json = [
	{
		"name": "John Doe",
...
# sample product data to be seeded into the Products table
product_json = [
	{
...
# function to recreate the database by dropping all tables and creating them again
def recreate_db():
...
def seeder():
	# seeding user data into the Users table
	for user in user_json:
		Users.create(user.get("name"), user.get("email"), user.get("password"), user.get("contact"))

	# seeding product data into the Products table
	...
	# seeding customer data from a CSV file into the Customer table
	...
	# seeding supplier data from a CSV file into the Supplier table
	...
	# seeding company products data from a CSV file into CompanyProducts table
	...
	# seeding company orders data from a CSV file into the CompanyOrders table
	...
	# seeding order items data from a CSV file into the OrderItems table
...
# custom command to seed and recreate the database
@cli.command()
def rsd():
